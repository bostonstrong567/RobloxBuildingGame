...
function Handles:CreateHandles(Options)
	...
	local function IsInHandle(InputObject)
		...
		if self.Handles then
			...
			for _, Handle in self.Handles do
				local EndPoint = Handle.AbsolutePosition + Handle.AbsoluteSize

				local LeftCheck = X >= Handle.AbsolutePosition.X
				local RightCheck = X <= EndPoint.X
				local TopCheck = Y >= Handle.AbsolutePosition.Y
				local BottomCheck = Y <= EndPoint.Y

				local IsInside = LeftCheck and RightCheck and TopCheck and BottomCheck

				--	Handle.ImageTransparency = IsInside and 0 or 0.33
	...
	self.Maid["HoverChanged"] = Support.AddUserInputListener('Changed', {'MouseMovement', 'Touch'}, true, IsInHandle)
	...
	for _, Side in ipairs(Enum.NormalId:GetEnumItems()) do
		...
		Handle.ImageColor3 = Options.Color or Color3.new(1, 0.8, 0)
		...
		HandleMaid[Side.Name .. "DragStart"] = Support.AddUserInputListener('Began', {'MouseButton1', 'Touch'}, true, function (InputObject)
			if CurrentHandle ~= Handle then
				return
			end

			Handle:AddTag("STATE_Press")

			local X = InputObject.Position.X + self.GuiInset.X
			local Y = InputObject.Position.Y + self.GuiInset.Y
			...
			HandleMaid[Side.Name .. "Dragging"] = DragMaid
			...
			DragMaid.DragEnd = Support.AddUserInputListener('Ended', {'MouseButton1', 'Touch'}, true, function ()
				HandleMaid[Side.Name .. "Dragging"] = nil
			end)

			-- Fire callback when dragging ends
			DragMaid.Callback = function ()
				task.spawn(Options.OnDragEnd)
			end

		end)

		-- Finish dragging when input ends while aiming at handle
		HandleMaid[Side.Name .. "InHandleDragEnd"] = Support.AddUserInputListener('Ended', {'MouseButton1', 'Touch'}, true, function ()
			HandleMaid[Side.Name .. "Dragging"] = nil
...
function Handles:Resume()
	...
	for Side, Handle in self.Handles do
		local UnitVector = Vector3.FromNormalId(Side)
		task.spawn(function ()
			while self.Running do
				if self.Adornee ~= nil and self.Adornee.Parent ~= nil then
...
function Handles:UpdateHandle(Handle, SideUnitVector)
	...
	local AdorneeCFrame, AdorneeSize
	if self.IsAdorneeModel then
		AdorneeCFrame, AdorneeSize = self.Adornee:GetBoundingBox()
	elseif self.Adornee:IsA("Attachment") then
		AdorneeCFrame = self.Adornee.WorldCFrame
		AdorneeSize = Vector3.new(0.2, 0.2, 0.2)
	else
		AdorneeCFrame = self.Adornee.CFrame
		AdorneeSize = self.Adornee.Size
	end
